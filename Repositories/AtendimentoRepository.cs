...
namespace SistemaAtendimento.Repositories
{
    public class AtendimentoRepository
    {
        ...
        {
            var Lista = new List<Atendimentos>(); // deixando o var eu deixo generico 
            using (var conexao = ConexaoDB.GetConexao())
            { 
            string sql = @"SELECT  a.*,  c.nome AS cliente_nome,    u.nome AS usuario_nome,    s.nome 
                        AS situacao_nome, c.cpf_cnpj FROM atendimentos a INNER JOIN clientes c 
                       ON c.id = a.cliente_id INNER JOIN usuarios u     ON u.id = a.usuario_id
                    ...
                    using (var comando = new SqlCommand(sql,conexao))
                    {
                        ...
                        using (var linhas = comando.ExecuteReader())
                        {
                            Lista.Add(new Atendimentos
                            {
                                ...
                                SituacaoAtendimentoId = linhas["situacao_atendimento_id"].ToString(),
        ...
        public void Inserir(Atendimentos atendimento)
        {
            using (var conexao = ConexaoDB.GetConexao())
            {
                String sql = @"INSERT INTO atendimentos (cliente_id, usuario_id, data_abertura, data_fechamento, observacao, situacao_atedimento_id) VALUES
                            (@cliente_id, @usuario_id, @data_abertura, @data_fechamento, @observacao, @situacao_atedimento_id)";
                ...
                using (var comando = new SqlCommand(sql, conexao))
                {
                    ...
                    comando.Parameters.AddWithValue("@situacao_atedimento_id", atendimento.SituacaoAtendimentoId);
                    conexao.Open();
        ...
        public void Atualizar(Atendimentos atendimento)
        {
            using ( var conexao = ConexaoDB.GetConexao())
            {
                String sql = @"UPDATE atendimentos SET cliente_id = @cliente_id, usuario_id = @usuario_id, data_abertura = @data_abertura, data_fechamento = @data_fechamento, observacao = @observacao, situacao_atedimento_id = @situacao_atedimento_id
                            WHERE id = @id";
                using (var comando = new SqlCommand(sql, conexao))
                {
